...
% ===== Kompatybilne z pdflatex (i XeLaTeX) =====
...
Temat 5: Ewolucja ekosystemów języków programowania\\[0.4em]
\normalsize Repozytorium: \url{https://github.com/McEnigma24/UMISI-sem1-Visualization}}
\date{\today}
...
\texttt{data/raw/real/manyLanguages\_added.csv}.
...
\midrule
1 & \texttt{etl.py} & wczytanie i~filtrowanie danych, udziały, metryki, koncentracja \\
2 & \texttt{build\_vectors.py} & długi format wektorów cech (profil czasowy języka) \\
3 & \texttt{dim\_reduction.py} & 7 metod redukcji wymiaru w~2D i~3D \\
4 & \texttt{eda.py} & statystyki per język, klastry $k$-means, wariancja PCA, profile \\
5 & \texttt{build\_viz.py} & główny generator: Altair $\rightarrow$ Vega-Lite, Plotly 3D, montaż \texttt{index.html} \\
6 & \texttt{build\_yearly\_viewer.py} & strona rocznego podglądu (treemap / siatka) \\
7 & \texttt{build\_market\_snapshot\_viewer.py} & strona mapy udziałów (kwartalna) \\
8 & \texttt{build\_concentration\_viewer.py} & strona koncentracji rynku \\
\bottomrule
\end{tabular}
\end{center}
...
Domyślnym wejściem ETL jest \texttt{data/raw/real/manyLanguages\_added.csv} (eksport BigQuery);
inny plik: \texttt{python src/etl.py -{}-input ścieżka.csv}. Wyniki metod stochastycznych
(klastrowanie $k$-means, t-SNE, UMAP itd.) są powtarzalne dzięki ustalonemu ziarnu
\texttt{RANDOM\_SEED = 42} w~\texttt{src/config.py}.
