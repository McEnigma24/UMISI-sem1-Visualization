...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; }}
    ...
    .viewer {{ max-width: 920px; margin: 0 auto; padding: 0.5rem 1rem 1.25rem; }}
    .slideshow-rows {{
      display: flex;
    ...
    .event-marker-hit {{ cursor: pointer; pointer-events: all; }}
    .chart-tooltip {{
      position: fixed;
      z-index: 8000;
      left: 0;
      ...
      box-shadow: 0 14px 40px rgba(0, 0, 0, 0.28), 0 0 0 1px rgba(255, 255, 255, 0.06) inset;
      font-size: 0.84rem;
      line-height: 1.45;
      pointer-events: none;
      opacity: 0;
    ...
    .chart-tooltip-kind.kind-1 {{ color: #fcd34d; }}
    .chart-tooltip-kind.kind-2 {{ color: #a5b4fc; }}
    ...
    .chart-tooltip-body p:last-child {{ margin-bottom: 0; }}
  </style>
</head>
<body>
  <div class="viewer">
    <div class="slideshow-rows" id="slideshowControls">
      <div class="slideshow-row slideshow-toolbar-line">
        ...
        <div class="speed-cluster">
          <label>Predkosc (s)
            <input type="range" id="speedRange" min="10" max="200" step="1" value="30" title="0.10 s do 2.00 s" />
            <span id="speedVal" class="speed-val">0.30</span>
          </label>
        </div>
    ...
    <div id="stage"></div>
    <div id="chartTooltip" class="chart-tooltip" hidden role="tooltip">
      <div class="chart-tooltip-kind" id="chartTooltipKind"></div>
      <div class="chart-tooltip-period" id="chartTooltipPeriod"></div>
      <div class="chart-tooltip-body" id="chartTooltipBody"></div>
    </div>
  ...
  <script>
      ...
      _vhRaf = requestAnimationFrame(() => {{
        ...
        try {{
          ...
          if (window.parent && window.parent !== window) {{
            window.parent.postMessage({{ type: "umisi-viewer-resize", source: "concentration-viewer", height: h }}, "*");
          }}
        }} catch (e) {{}}
      }});
    }}
    ...
    function slideshowSecondsFromInput() {{
      ...
      const v = r ? parseInt(r.value, 10) : 30;
      return (Number.isFinite(v) ? v : 30) / 100;
    }}
    ...
    /** Zwraca obiekt do ladnego tooltipu lub null */
    function buildEventTooltipParts(idx) {{
      const cur = pointAt(idx);
      const prev = pointAt(idx - 1);
      if (!prev) return null;
      ...
      if (kind === 1) {{
        ...
        const lines = [];
        lines.push("Zmiana skladu top " + topN + ".");
        if (added.length) lines.push("Wchodza: " + added.join(", ") + ".");
        if (removed.length) lines.push("Wychodza: " + removed.join(", ") + ".");
        return {{ period: String(cur.period), kind: 1, kindLabel: "Zmiana skladu", lines }};
      }}
      if (kind === 2) {{
        return {{
          period: String(cur.period),
          kind: 2,
          kindLabel: "Zmiana kolejnosci",
          lines: [
            "Bylo: " + prev.languages.join(" > "),
            "Jest: " + cur.languages.join(" > "),
          ],
        }};
      }}
      return null;
    ...
    function positionChartTooltip(el, clientX, clientY) {{
      const pad = 10;
      const margin = 14;
      el.hidden = false;
    ...
    function bindChartTooltips() {{
      ...
      const kindEl = document.getElementById("chartTooltipKind");
      const periodEl = document.getElementById("chartTooltipPeriod");
      const bodyEl = document.getElementById("chartTooltipBody");
      if (!tip || !stage) return;
      ...
      stage.querySelectorAll(".event-marker").forEach(g => {{
        const idx = parseInt(g.getAttribute("data-event-idx"), 10);
        const hit = g.querySelector(".event-marker-hit");
        if (!hit || Number.isNaN(idx)) return;
        hit.addEventListener("mouseenter", ev => {{
          clearTimeout(_tipHideTimer);
          const parts = buildEventTooltipParts(idx);
          if (!parts) return;
          kindEl.textContent = parts.kindLabel;
          kindEl.className = "chart-tooltip-kind kind-" + parts.kind;
          periodEl.textContent = parts.period;
          bodyEl.innerHTML = parts.lines.map(line => "<p>" + escapeHtml(line) + "</p>").join("");
          positionChartTooltip(tip, ev.clientX, ev.clientY);
        }});
        hit.addEventListener("mousemove", ev => {{
          if (!tip.hidden) positionChartTooltip(tip, ev.clientX, ev.clientY);
        }});
        hit.addEventListener("mouseleave", () => {{
          _tipHideTimer = setTimeout(hideTip, 40);
        }});
      }});
    }}
    ...
    function renderChart(upToIdx) {{
      ...
      yTicks().forEach(v => {{
        ...
        s += `<text x="${{M.left - 8}}" y="${{y}}" text-anchor="end" dominant-baseline="middle" font-size="11" font-weight="600" fill="#111">${{v}}%</text>`;
      }});
      ...
      PERIODS.forEach((p, i) => {{
        ...
        const x = xPos(i);
        s += `<text x="${{x}}" y="${{H - 10}}" text-anchor="middle" font-size="10" font-weight="600" fill="#111">${{p}}</text>`;
      }});
      ...
      for (let idx = 1; idx <= upToIdx; idx++) {{
        ...
        if (kind === 1) {{
          s += `<g class="event-marker" data-event-idx="${{idx}}">`;
          s += `<circle cx="${{x}}" cy="${{y}}" r="${{r}}" fill="#f59e0b" stroke="#b45309" stroke-width="2"/>`;
          s += `<circle cx="${{x}}" cy="${{y}}" r="2" fill="#fff"/>`;
          s += `<circle cx="${{x}}" cy="${{y}}" r="14" fill="transparent" class="event-marker-hit"/>`;
          s += `</g>`;
        }} else {{
      ...
      s += `<text x="${{M.left}}" y="16" font-size="13" font-weight="600" fill="#111">Laczny udzial top N (%)</text>`;
      s += `<text x="${{M.left + INNER_W}}" y="16" font-size="10" font-weight="500" fill="#222" text-anchor="end">`;
      s += `Kolo pom.: zmiana skladu. Romby: tylko kolejnosc — najedz (szybki opis przy kursorze).</text>`;
      s += `</svg>`;
      stage.innerHTML = s;
      bindChartTooltips();
    }}
